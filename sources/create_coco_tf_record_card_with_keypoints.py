...
_COCO_KEYPOINT_NAMES = [
    b'card_angle_ul', b'card_angle_ur', b'card_angle_dr', b'card_angle_dl'
]
...
def create_tf_example(image,
  ...
  for object_annotations in annotations_list:
    ...
    print(f"debug: include_keypoint = {include_keypoint}")
    ...
    if include_keypoint:
      ...
      if annotation_id in keypoint_annotations_dict:
        ...
        print(f"debug: num_kpts = {num_kpts}")
        ...
        print(f"debug: kpts = {_COCO_KEYPOINT_NAMES}")
        ...
        print(f"debug: kpts0 = {_COCO_KEYPOINT_NAMES}")

        num_keypoints.append(0)

    print(f"debug: include_densepose = {include_densepose}")
...
def _create_tf_record_from_coco_annotations(annotations_file, image_dir,
  ...
  with contextlib2.ExitStack() as tf_record_close_stack, \
    ...
    if keypoint_annotations_file:
      print(f"debug: keypoint_annotations_file = {keypoint_annotations_file}")
      with tf.gfile.GFile(keypoint_annotations_file, 'r') as kid:
        # tf_read = str(kid.read())
        # print(f"debug: kid = {str(tf_read)}")
        keypoint_groundtruth_data = json.load(kid)
        # print(f"debug: keypoint_groundtruth_data = {keypoint_groundtruth_data}")
    ...
    for idx, image in enumerate(images):
      ...
      keypoint_annotations_dict = None
      print(f"debug: keypoint annotaion file = {keypoint_annotations_file}")
      if keypoint_annotations_file:
