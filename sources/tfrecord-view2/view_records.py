...
def view_records(file_path, class_labels, stride = 1, verbose = 1):
    ...
    """
    dataset = tf.compat.v1.data.TFRecordDataset([file_path])
    ...
    for im_ind in range(num_records):
        ...
        labels =  tf.compat.v1.sparse_tensor_to_dense(parsed_example['image/object/class/label'], default_value=0).numpy()
        x1norm =  tf.compat.v1.sparse_tensor_to_dense( parsed_example['image/object/bbox/xmin'], default_value=0).numpy()
        x2norm =  tf.compat.v1.sparse_tensor_to_dense( parsed_example['image/object/bbox/xmax'], default_value=0).numpy()
        y1norm =  tf.compat.v1.sparse_tensor_to_dense( parsed_example['image/object/bbox/ymin'], default_value=0).numpy()
        y2norm =  tf.compat.v1.sparse_tensor_to_dense( parsed_example['image/object/bbox/ymax'], default_value=0).numpy()
...
if __name__ == '__main__':
    ...
    class_labels =  {}
    for i in range(100):
       class_labels[i] = int(i)

    print(class_labels)

    #Make the following using voc_to_tfr.py
    data_path = r"/home/joefox/Downloads/my_faster_rcnn_resnet50_v1_640x640_coco17_tpu-8/data/train.record"
